...
2 `.pem` files will be generated in the **_current working folder_** as defined in the command executed
...
In case an existing "*configuration.yml*" file is available in the Authelia folder, make a backup copy of that, or simply download the sample one from [here](/configuration.yml) and make sure to change all CAPITALISED parameters to match your setup
...
A new file will be generated named **_key.pem_**, copy the contents of this file into the above configuration and then run the container. All should be fine now
